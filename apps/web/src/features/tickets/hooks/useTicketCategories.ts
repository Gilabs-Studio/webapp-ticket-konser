...
export function useTicketCategoriesByEventIdPublic(eventId: string) {
  return useQuery({
    queryKey: ["ticket-categories", "public", "event", eventId],
    queryFn: () => ticketCategoryService.getTicketCategoriesByEventIdPublic(eventId),
    enabled: !!eventId,
    staleTime: 30000,
  });
}
